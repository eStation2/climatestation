...
folder are mapped to the application's root namespace, 'climatestation'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "climatestation"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'climatestation' in the same way
as `"climatestation/sass/src"`.
