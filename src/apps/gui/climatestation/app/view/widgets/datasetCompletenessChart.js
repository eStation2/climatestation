...
Ext.define("climatestation.view.widgets.datasetCompletenessChart",{
    ...
    requires: [
        ...
        'climatestation.view.widgets.datasetCompletenessChartController',
        'climatestation.view.widgets.datasetCompletenessChartModel'
    ],
    ...
    ,drawCompletenessChart:function(data) {
        ...
        if (mapsetdatasetrecord.get('nodisplay')  == 'no_minutes_display'){
            ...
            seriestitle = '<span style="color:#f78b07">' + climatestation.Utils.getTranslation('no_minutes_display') + '</span>';
            ...
            totfiles = climatestation.Utils.getTranslation('no_minutes_display');
            missingfiles = '';
            firstdate = '';
            lastdate = '';
            spriteXposition = 30;
            ...
            dataObj["data1"] = '100'; // 100%
            datasetdata.push(dataObj);
            seriestitle = '<span style="color:#81AF34">'+ climatestation.Utils.getTranslation('singlefile') + '</span>';
            seriestitles.push(seriestitle);
            seriescolors.push('#81AF34'); // green

            // Update the 4 sprites (these are not reachable through getSprites() on the chart)
            totfiles = climatestation.Utils.getTranslation('files') + ': ' + completeness.totfiles;
            if(completeness.missingfiles>0)
                missingfiles = climatestation.Utils.getTranslation('Missing') + ': ' + completeness.missingfiles;
            firstdate = '';
            lastdate = '';
        }
            ...
            dataObj["data1"] = '100'; // 100%
            datasetdata.push(dataObj);
            seriestitle = '<span style="color:#808080">'+ climatestation.Utils.getTranslation('notanydata') + '</span>';
            seriestitles.push(seriestitle);
            seriescolors.push('#808080'); // gray

            // Update the 4 sprites (these are not reachable through getSprites() on the chart)
            totfiles = climatestation.Utils.getTranslation('notanydata');
            missingfiles = '';
            firstdate = '';
            lastdate = '';
        }
            ...
            completeness.intervals.forEach(function (interval) {
                ...
                seriestitle = '<span style="color:'+color+'">' + climatestation.Utils.getTranslation('from') + ' ' + interval.fromdate + ' ' + climatestation.Utils.getTranslation('to') + ' ' + interval.todate + ' - ' + intervaltype + '</span></br>';
                seriestitles.push(seriestitle);
            });
