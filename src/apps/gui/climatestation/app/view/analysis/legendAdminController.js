Ext.define('climatestation.view.analysis.legendAdminController', {
    ...
    requires: [
        ...
        'climatestation.view.analysis.addEditLegend'
    ],
    ...
    ,copyLegend: function(){
        ...
        if (!climatestation.Utils.objectExists(selrec)){
            Ext.toast({html: climatestation.Utils.getTranslation('nolegend_selected'), title: climatestation.Utils.getTranslation('copy_legend'), width: 200, align: 't'});   // 'Copy legend', 'No legend selected!'
        }
        else {
            Ext.Ajax.request({
               ...
               success: function(result, request) {
                   ...
                   if (returnData.success){
                       ...
                       //  };
                       // var newrecord = new climatestation.model.Legend(defaultData);
                       // legendStore.add(newrecord);
                       this.loadLegendsStore();

                       Ext.toast({html: climatestation.Utils.getTranslation('legend_name')+': <b>'+returnData.legend_descriptive_name+'</b>', title: climatestation.Utils.getTranslation('legend_copied'), width: 200, align: 't'});
    ...
    ,newLegend: function(){
        ...
        var newLegendWin = new climatestation.view.analysis.addEditLegend({
            params: {
                new: true,
                edit: false,
                view: false,
    ...
    ,editLegend: function(grid, rowIndex){
        ...
        var edit = false;
        var view = true;
        if (record.get('defined_by') != 'JRC' || (climatestation.Utils.objectExists(user) && user.userlevel == 1)){
            edit = true;
            view = false;
        }

        //if (record.get('defined_by') != 'JRC') {
            var editLegendWin = new climatestation.view.analysis.addEditLegend({
                params: {
                    new: false,
                    edit: edit,
                    view: view,
    ...
    ,deleteLegend: function(grid, rowIndex, row){
        ...
        // if (record.get('deletable')){
            Ext.Msg.show({
                title: climatestation.Utils.getTranslation('deletelegendquestion'),     // 'Delete legend definition?',
                message: climatestation.Utils.getTranslation('deletelegendquestion') + ' "' + record.get('legend_descriptive_name') + '"?',
