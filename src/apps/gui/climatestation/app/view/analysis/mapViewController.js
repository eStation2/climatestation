Ext.define('climatestation.view.analysis.mapViewController', {
    ...
    requires: [
        ...
        'climatestation.view.analysis.ProductNavigator'
    ],
    ...
    getMapSettings: function(){
        ...
        me.map.getLayers().getArray().forEach(function (layer,idx){
            if (climatestation.Utils.objectExists(layer.get("layerdb_id")) && layer.get("visible")){
        ...
        var mapSettings = {
            ...
            productcode: climatestation.Utils.objectExists(me.productcode) && me.productcode != '' ? me.productcode : null,
            subproductcode: climatestation.Utils.objectExists(me.subproductcode) && me.subproductcode != '' ? me.subproductcode : null,
            productversion: climatestation.Utils.objectExists(me.productversion) && me.productversion != '' ? me.productversion : null,
            mapsetcode: climatestation.Utils.objectExists(me.mapsetcode) && me.mapsetcode != '' ? me.mapsetcode : null,
            productdate: me.productdate,
            legendid: climatestation.Utils.objectExists(me.legendid) && me.legendid > 0 ? me.legendid : null,
    ...
    ,setMapTemplateName: function(){
        ...
        if (me.getView().isNewTemplate){
            ...
            if (climatestation.Utils.objectExists(me.getView().templatename) && me.getView().templatename != ''){
                newMapTemplateName = me.getView().templatename + ' - copy';
            }
            else {
                if (climatestation.Utils.objectExists(me.getView().productname) && me.getView().productname != ''){
            ...
            var messageBox = Ext.create('Ext.form.Panel', {
                title: climatestation.Utils.getTranslation('map_tpl_name'),
                bodyPadding: 5,
                width: 350,
                floating: true,
                floatable: true,
                ...
                items: [{
                    fieldLabel: climatestation.Utils.getTranslation('map_tpl_save_message') + ':',
                    labelAlign: 'top',
...
//             });
//
//             messageBox.show(climatestation.Utils.getTranslation('map_tpl_name'), climatestation.Utils.getTranslation('map_tpl_save_message') + ':', function(btn, text){   // 'Map Template Name'   'Please give a unique name for the new map template'
//                 // if (text == ''){
...
//             }, this, false, newMapTemplateName);
        ...
        Ext.Ajax.request({
            ...
            success: function (response, request) {
                ...
                if (responseJSON.success){
                    Ext.toast({hideDuration: 2000, html: responseJSON.message, title: climatestation.Utils.getTranslation('save_map_tpl'), width: 300, align: 't'});     // "Save Map template"
    ...
    ,__saveMapTemplate: function(){
        ...
        var layerdb_ids = []
        me.map.getLayers().getArray().forEach(function (layer,idx){
            if (climatestation.Utils.objectExists(layer.get("layerdb_id"))){
                layerdb_ids.push(layer.get("layerdb_id"));
            }
        });
    ...
    ,refreshTitleData: function(){
        ...
        if (climatestation.Utils.objectExists(me.productsensor) && me.productsensor.trim() != ''){
        ...
        if (climatestation.Utils.objectExists(me.productcode)){
            productcodetitle = ' <b class="smalltext">' + me.productcode;
            if (climatestation.Utils.objectExists(me.subproductcode)){
                productcodetitle += ' - ' + me.subproductcode ;
            }
            productcodetitle += '</b>';
        }

        if (climatestation.Utils.objectExists(me.productversion) && me.productversion != 'undefined'){
            versiontitle = ' - ' +  ' <b class="smalltext">' + me.productversion + '</b>';
        }
        if (climatestation.Utils.objectExists(me.mapsetcode)){
    ...
    ,addProductLayer: function(productcode, productversion, mapsetcode, subproductcode, tpl_productdate, legendid,
        ...
        Ext.Ajax.request({
            ...
            params: params,
            loadMask:climatestation.Utils.getTranslation('loadingdata'),  // 'Loading data...',
            scope: me,
            success:function(response, request ){
                var responseJSON = Ext.util.JSON.decode(response.responseText);
                ...
                if (climatestation.Utils.objectExists(tpl_productdate) &&  tpl_productdate != ''){
                    ...
                    if (tpl_productdateExists){
                        ...
                        Ext.toast({ html: climatestation.Utils.getTranslation('date_maptpl_not_avail'), title: climatestation.Utils.getTranslation('date_not_available'), width: 300, align: 't' });
                    }
                }
                ...
                me.productlayer = new ol.layer.Tile({       // ol.layer.Tile   or  ol.layer.Image
                    title: climatestation.Utils.getTranslation('productlayer'),  // 'Product layer',
                    ...
                    source: new ol.source.TileWMS({    // ol.source.TileWMS or ol.source.ImageWMS
                        ...
                        //     html: '&copy; <a href="https://ec.europa.eu/jrc/">'+climatestation.Utils.getTranslation('estation2')+'</a>'
                        // })],
                        params: {
                            productcode:productcode,
                            productversion:productversion,
    ...
    ,updateProductLayer: function() {
        ...
        if (climatestation.Utils.objectExists(me.productdate_linked)){
            var pattern = /(\d{4})(\d{2})(\d{2})/;
            //if (me.date_format == 'MMDD') {
        ...
        if (me.link_product_layer){
            Ext.Object.each(mapViewWindows, function(id, mapview_window, thisObj) {
                if (me.workspace == mapview_window.workspace){
                    if (me != mapview_window && mapview_window.link_product_layer && climatestation.Utils.objectExists(mapview_window.productcode)){
    ...
    ,clearSelectedFeatureOverlayInOtherMapViews: function () {
        ...
        Ext.Object.each(mapViewWindows, function(id, mapview_window, thisObj) {
            if (me.workspace == mapview_window.workspace) {
                if (me != mapview_window && climatestation.Utils.objectExists(mapview_window.selectedFeatureOverlay)) {
    ...
    ,redrawTimeLine: function () {
        ...
        if (mapviewtimeline.hidden == false && climatestation.Utils.objectExists(mapview_timelinechart_container.timelinechart)){
    ...
    ,toggleOutmask: function(btn, event) {
        ...
        if (btn.pressed) {
            if (climatestation.Utils.objectExists(mapviewwin.selectedfeature)){
                mapviewwin.getController().outmaskFeature();
                if (climatestation.Utils.objectExists(mapviewwin.selectedFeatureOverlay)) {
    ...
    ,openProductNavigator: function() {
        ...
        if (!me.productnavigator){
            me.productnavigator = new climatestation.view.analysis.ProductNavigator({mapviewid:me.getId(), owner:me});
    ...
    ,addLayerSwitcher: function (map){
        ...
        if (!layerswitcherexists) {
            var layerSwitcher = new ol.control.LayerSwitcher({
                tipLabel: climatestation.Utils.getTranslation('layers')   // 'Layers' // Optional label for button
            });
    ...
    ,addVectorLayerToMapView: function(layerrecord){
        ...
        var namefield = layerrecord.get('feature_display_column').split(',')[0]
            ,vectorlayer_idx = -1
            ,layertitle = climatestation.Utils.getTranslation(layerrecord.get('layername')) + '</BR><b class="smalltext" style="color:darkgrey;">' + climatestation.Utils.getTranslation(layerrecord.get('provider')) +'</b>';
            ...
            //,highlight_fillcolor_opacity = 'rgba(' + climatestation.Utils.HexToRGB(layerrecord.get('feature_highlight_fillcolor')) + ',' + fillopacity + ')';
        ...
        if (addToPool){
            var myLoadMask = new Ext.LoadMask({
                msg    : climatestation.Utils.getTranslation('loadingvectorlayer'),   // 'Loading vector layer...',
            ...
            var listenerKey = vectorSource.on('change', function(e) {
              if (vectorSource.getState() == 'ready') {
                ...
                // or vectorSource.unByKey(listenerKey) if you don't use the current master branch of ol3
                  ...
                  myLoadMask.hide();
                  Ext.toast({html: climatestation.Utils.getTranslation('error_loading_vectory_layer'), title: climatestation.Utils.getTranslation('error'), width: 300, align: 't'});
              }
              else {
                  myLoadMask.hide();
              }
        ...
        var fillopacity = (layerrecord.get('feature_highlight_fillopacity')/100).toString().replace(",", ".")
            ...
            ,pointhighlight_fillcolor_opacity = 'rgba(' + climatestation.Utils.HexToRGB(layerrecord.get('feature_highlight_fillcolor')) + ',' + pointfillopacity + ')'
    ...
    ,displayFeatureInfo: function(pixel) {
        ...
        if (climatestation.Utils.objectExists(topfeature)) {
            if (topfeature !== me.highlight) {
                ...
                if (me.toplayer.get('layer_id') == 'drawvectorlayer'){      // regionname_html == 'undefined' &&
                    regionname_html = climatestation.Utils.getTranslation('drawn') + " " + topfeature.getGeometry().getType();      // "Drawn "
                }
                regionname.setHtml(regionname_html);

                if (featureOverlay != null){
                    if (climatestation.Utils.objectExists(me.featureOverlay)) {
    ...
    ,displaySelectedFeatureInfo: function(pixel,displaywkt) {
        ...
        if (climatestation.Utils.objectExists(feature)) {
            if (feature !== me.selectedfeature) {
                ...
                if (me.toplayer.get('layer_id') == 'drawvectorlayer'){      // regionname_html == 'undefined' &&
                    ...
                    regionname_html = climatestation.Utils.getTranslation('drawn') + " " + feature.getGeometry().getType();     // "Drawn "
                }
                ...
                if (selectedFeatureOverlay != null){
                    if (climatestation.Utils.objectExists(me.selectedFeatureOverlay)) {
    ...
    ,editLayerDrawProperties: function(callComponent){
        ...
        var BorderDrawPropertiesWin = new Ext.Window({
            ...
            ,title: climatestation.Utils.getTranslation('Draw properties ') + climatestation.Utils.getTranslation(layerrecord.get('layername'))   // climatestation.Utils.getTranslation(layerrecord.get('menu')) + ' ' + (layerrecord.get('submenu') != '' ? layerrecord.get('submenu') + ' ' : ' ') + climatestation.Utils.getTranslation(layerrecord.get('layername'))
            ,width:420
            ,plain: true
            ,modal: true
            ,resizable: true
            ...
            ,items:[{
                ...
                source: {
                    polygon_outlinecolor: climatestation.Utils.convertRGBtoHex(layerrecord.get('polygon_outlinecolor')),
                    polygon_outlinewidth: layerrecord.get('polygon_outlinewidth'),
                    feature_highlight_outlinecolor: climatestation.Utils.convertRGBtoHex(layerrecord.get('feature_highlight_outlinecolor')),
                    feature_highlight_outlinewidth: layerrecord.get('feature_highlight_outlinewidth'),
                    feature_highlight_fillopacity: layerrecord.get('feature_highlight_fillopacity'),
                    feature_highlight_fillcolor: climatestation.Utils.convertRGBtoHex(layerrecord.get('feature_highlight_fillcolor')),
                    feature_selected_outlinecolor: climatestation.Utils.convertRGBtoHex(layerrecord.get('feature_selected_outlinecolor')),
                    feature_selected_outlinewidth: layerrecord.get('feature_selected_outlinewidth')
                },
                sourceConfig: {
                    polygon_outlinecolor: {
                        displayName: climatestation.Utils.getTranslation('outlinecolour'),   // 'Outline colour',
                        editor: {
                    ...
                    polygon_outlinewidth: {
                        displayName: climatestation.Utils.getTranslation('oulinewidth'),   // 'Outline width',
                        type: 'number'
                    },
                    feature_highlight_outlinecolor: {
                        displayName: climatestation.Utils.getTranslation('highlightoutlinecolour'),   // 'Highlight outline colour',
                        editor: {
                            xtype: 'mycolorselector'    // 'mycolorpicker'
                            //,floating: false
                        }
                    ...
                    feature_highlight_outlinewidth: {
                        displayName: climatestation.Utils.getTranslation('highlightoutlinewidth'),   // 'Highlight outline width',
                        type: 'number'
                    },
                    feature_highlight_fillcolor: {
                        displayName: climatestation.Utils.getTranslation('highlightfillcolour'),   // 'Highlight fill colour',
                        editor: {
                            xtype: 'mycolorselector'    //'mycolorpicker'
                            //,floating: false
                        }
                    ...
                    feature_selected_outlinecolor: {
                        displayName: climatestation.Utils.getTranslation('selectedfeatureoutlinecolour'),   // 'Selected feature outline colour',
                        editor: {
                            xtype: 'mycolorselector'    // 'mycolorpicker'
                            //,floating: false
                        }
                    ...
                    feature_highlight_fillopacity: {
                        displayName: climatestation.Utils.getTranslation('highlightfillopacity'),   // 'Highlight fill opacity',
                        editor: {
                            xtype: 'combobox',
                    ...
                    feature_selected_outlinewidth: {
                        displayName: climatestation.Utils.getTranslation('selectedfeatureoutlinewidth'),   // 'Selected feature outline width',
                        type: 'number'
                    }
                },
                //customEditors: {
    ...
    ,createLayersMenuItems: function(mainmenuitem) {
        ...
        layersStore.each(function(layer) {
            ...
            if (layer.get('submenu')!= '' && layer.get('submenu') != currentSubmenu) {
                ...
                newSubMenu = {
                    text: climatestation.Utils.getTranslation(layer.get('submenu')),
            ...
            var checkboxCmp = Ext.create('Ext.form.field.Checkbox', {
                //boxLabel: climatestation.Utils.getTranslation(layer.get('submenu')) + (layer.get('submenu') != '' ? ' ' : '') + climatestation.Utils.getTranslation(layer.get('layerlevel')),
                boxLabel: climatestation.Utils.getTranslation(layer.get('layername')) + '</BR><b class="smalltext" style="color:darkgrey;">' + climatestation.Utils.getTranslation(layer.get('provider')) +'</b>',
                // flex: 1,
                autoWidth: true,
                maxWidth: 170,
                minWidth: 100,
            ...
            containerItems.push({
                ...
                // columnWidth: 0.10,
                text: '',
                tooltip: climatestation.Utils.getTranslation('tipeditlayerdrawproperties'),   // 'Edit layer draw properties.',
                iconCls: 'edit16',
    ...
    ,createToolBar: function() {
        ...
        var layersmenubutton = {
            ...
            menu: {
                ...
                items: [{
                    text: climatestation.Utils.getTranslation('borderlayers'),   // 'Border layers (FAO Gaul 2015)',
                    name: 'border',
                    ...
                    text: climatestation.Utils.getTranslation('marinelayers'),   // 'Marine vector layers',
                    name: 'marine',
                    menu: {
                        defaults: {
                            hideOnClick: false,
                    ...
                    text: climatestation.Utils.getTranslation('otherlayers'),   // 'Other vector layers',
                    name: 'other',
                    menu: {
                        defaults: {
                            hideOnClick: false,
        ...
        me.tbar = Ext.create('Ext.toolbar.Toolbar', {
            ...
            items: [{
                ...
                //     //     productNavigatorWin = new climatestation.view.analysis.ProductNavigator({mapviewid:btn.up().up().getId()});
                //     // }
                ...
                //         me.mapViewProductNavigator = new climatestation.view.analysis.ProductNavigator({mapviewid:me.getId(), owner:btn});
                //     }
                // }
            }
            ,layersmenubutton
                    ...
                    items: [
                        geometrytypescombo,
                        {
                            ...
                            handler: function(){
                                ...
                                var geojsonStr = '';
                                var drawnLayerName = climatestation.Utils.getTranslation('drawn_layer'); // 'Drawn layer';
                                ...
                                if (me.drawvectorlayer.getSource().getFeatures().length == 0){
                                    Ext.toast({html: climatestation.Utils.getTranslation('please_draw_geom_to_save'), title: climatestation.Utils.getTranslation('no_geom_to_save'), width: 300, align: 't'});    // 'Please draw at least one geometry to save as a layer.' "No geometries drawn to save!"
                                }
                                else {
                                    Ext.MessageBox.prompt(climatestation.Utils.getTranslation('drawn_layer_name'), climatestation.Utils.getTranslation('drawn_layer_save_message') + ': ', function(btn, text){   // 'Drawn layer Name'  'Please give a name to drawn layer'
                                        if (btn == 'ok'){
                                            params = {
