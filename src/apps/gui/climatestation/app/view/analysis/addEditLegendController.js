Ext.define('climatestation.view.analysis.addEditLegendController', {
    ...
    ,saveLegend: function(){
        ...
        if (this.validateClasses()){
            ...
            Ext.Msg.show({
                title: climatestation.Utils.getTranslation('savelegendquestion'),     // 'Save legend',
                message: climatestation.Utils.getTranslation('savelegendquestion') + ' "' + legenddescriptivename.getValue() + '"?',
                    ...
                    if (btn === 'ok') {
                        Ext.Ajax.request({
                           ...
                           params:{
                               ...
                               legendClasses:climatestation.Utils.makeGridJSON(legendClassesStore)
                           },
                           method: 'POST',
                           waitMsg:climatestation.Utils.getTranslation('savinglegend'), // 'Saving legend...',
                           scope:this,
                           success: function(result, request) {
                               ...
                               if (returnData.success){
                                    ...
                                    me.setTitle('<span class="panel-title-style">' + climatestation.Utils.getTranslation('editlegend') + '</span>');
                                    // legendclasses.onLoadClick(this.legendid);
                                    // var legendlist = Ext.getCmp('legendslist');
                                    // legendlist.store.load();
                                    Ext.Msg.show({
                                        title:climatestation.Utils.getTranslation('legend_saved'), // 'Legend saved',
                                        msg:climatestation.Utils.getTranslation('legend_saved'), // 'Legend saved successfully',
                                        modal:true,
                                        icon:Ext.Msg.INFO,
                                        buttons:Ext.Msg.OK
                                    });
                                   ...
                                   climatestation.Utils.showError(returnData.error || result.responseText);
                               }
            ...
            Ext.Msg.show({
                title:climatestation.Utils.getTranslation('warning'), // 'Warning!',
                msg:climatestation.Utils.getTranslation('check_legend_classes'), // 'Legend not saved! Check classes double values or overlaps!',
                modal:true,
                icon:Ext.Msg.ERROR,
                buttons:Ext.Msg.OK
            });
    ...
    ,newClass: function(){
        ...
        var newrecord = new climatestation.model.LegendClasses(defaultData);
    ...
    ,openColourPicker: function(record, el){
        ...
        var colourPickerWin = Ext.create('Ext.window.Window', {
            ...
            ,closable:true
            // ,loadMask:true
            // ,title:climatestation.Utils.getTranslation('colourpicker') // 'Colour picker'
            ,closeAction:'destroy'
            ,modal:true
            ...
            ,bbar: {
                ...
                items: ['->', {
                    ...
                    handler: function(){
                        ...
                        var rgbvalue = climatestation.Utils.HexToRGB(hexvalue);
            ...
            ,items: [{
                ...
                frame: false,
                format: "#HEX6",
                value: climatestation.Utils.convertRGBtoHex(record.get('color_rgb')),
                listeners: {
    ...
    ,generateClasses: function(){
        ...
        function setBackgroundColor (field) {
            ...
            var arr = [];
            if (climatestation.Utils.is_array(value)){
                arr = value;
            }
            else {
            ...
            var G = arr[1];
            var B = arr[2];
            var rgbValue = climatestation.Utils.RGBtoHex(R,G,B);
            var Rinverse = (R ^ 128);
            var Ginverse = (G ^ 128);
            var Binverse = (B ^ 128);

            var fontcolor = climatestation.Utils.RGBtoHex( Rinverse, Ginverse, Binverse);
            // console.info(field);
        ...
        if (generateClassesWin==null || generateClassesWin==='undefined' ) {
            ...
            generateClassesWin = Ext.create('Ext.window.Window', {
                ...
                ,closable:true
                ,loadMask:true
                ,title:climatestation.Utils.getTranslation('win_title_generate_classes') // 'Generate classes using color interpolation'
                ...
                ,bbar: ['->',{
                    ...
                    handler: function(){
                        ...
                        while (i<=classes-1){
                            ...
                            var newrecord = new climatestation.model.LegendClasses(classData);
                            legendClassesStore.add(newrecord);
                            // var lastrecordID = -1;
                            // if (legendClassesStore.data.length > 0) {   // Better if a method exists to insert as last
                ...
                ,items: [{
                    ...
                    items:[{
                        xtype:'numberfield',
                        reference:'classes',
                        fieldLabel: climatestation.Utils.getTranslation('classes'), // 'Classes',
                        width:90+90
                    },{
                        xtype:'numberfield',
                        reference:'startvalue',
                        fieldLabel: climatestation.Utils.getTranslation('start_value'), // 'Start value',
                        width:90+90
                    },{
                        xtype:'numberfield',
                        reference:'endvalue',
                        fieldLabel: climatestation.Utils.getTranslation('end_value'), // 'End value',
                        width:90+90
                    },{
                        xtype: 'container',
                        layout:'hbox',
                        ...
                        items: [{
                            ...
                            fieldLabel: climatestation.Utils.getTranslation('startcolour'), // 'Start colour',
                            ...
                            listeners:{
                                change:function(field,value) {
                                    ...
                                    var G = arr[1];
                                    var B = arr[2];
                                    var hexValue = climatestation.Utils.RGBtoHex(R,G,B);

                                    if (climatestation.Utils.objectExists(Ext.getCmp('startcolourpicker'))){
                            ...
                            // text:'',
                            tooltip:climatestation.Utils.getTranslation('opencolourpicker'), // 'Open colour picker',
                            // iconCls:'colorwheel',
                            // cls:'alignright',
                            scope:this,
                            width: 25,
                        ...
                        items: [{
                            ...
                            reference:'endcolour',
                            // renderer: setBackgroundColor,
                            fieldLabel: climatestation.Utils.getTranslation('endcolour'), // 'End colour',
                            regex: /(\d{1,3}) (\d{1,3}) (\d{1,3})/,
                            maskRe: /[0-9 ]+/,
                            invalidText: 'Not a valid RGB.  Must be 3 numbers between 0 and 255, devided by a space".',
                            listeners:{
                                change: function(field,value) {
                                    ...
                                    if (climatestation.Utils.objectExists(Ext.getCmp('endcolourpicker'))){
                        ...
                        fieldLabel: climatestation.Utils.getTranslation('apply_class_label'), // 'Apply class label',
                        labelSeparator: '',
                        value:true
                    }]
                }]
    ...
    ,generateLogValuesPanel: function(btn){
        ...
        var generateLogValuesPanel = new Ext.panel.Panel({
            ...
            ,closable: true
            ,closeAction: 'hide'
            ,title: climatestation.Utils.getTranslation('win_title_logarithmic_values') // 'Generate logarithmic values for existing classes'
            ,plain: true
            // ,modal:true
            ,frame: false
            ,border: false
            ...
            ,items: [{
                ...
                items:[{
                    xtype:'numberfield',
                    reference:'minvalue',
                    fieldLabel: climatestation.Utils.getTranslation('minvalue'), // 'Min value',
                    width:200,
                    minValue: 0.000001,
                    decimalPrecision:6
                },{
                    xtype:'numberfield',
                    reference:'maxvalue',
                    fieldLabel: climatestation.Utils.getTranslation('maxvalue'), // 'MAx value',
                    width:200,
                    minValue: 0.000001,
                    decimalPrecision:6
                }]
...
//        var legendHeader = '<tr><td colspan="'+TotClasses+'"><h2>'+legendname+'</h2></td></tr>';
        ...
        legendClassesStore.each( function (record){
             ...
             var G = arr[1];
             var B = arr[2];
             var color_html = climatestation.Utils.RGBtoHex(R,G,B);
//             if (TotClasses <= 1)
