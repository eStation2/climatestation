...
#  *************************************************************************    #
# ###############################################################################
    The Mann-Kendall Trend Test (sometimes called the MK test) is used to analyze
    time series data for consistently increasing or decreasing trends (monotonic trends).
    It is a non-parametric test, which means it works for all distributions
    (i.e. data doesn't have to meet the assumption of normality), but data should
    have no serial correlation. If the data has a serial correlation,
    it could affect in significant level (p-value).
    ...
    If the p-value of the test is lower than some significance level (common choices are 0.10, 0.05, and 0.01),
    then there is statistically significant evidence that a trend is present in the time series data.
"""
...
class TrendAnalyser(object):
    def __init__(self, data_series, time_series, frequency=12, alpha=0.05):  # , dbg=False, fid=None):
        """
    ...
    def _mk_test(self):
        ...
        if self.h:
            ...
            if self.trend_type != 'no trend':
                ...
                self.slope, self.intercept, r_value, p_linregress, std_err = stats.linregress(xx, test_trend)

    def _deseasonalize(self):
        """
        The timeseries is seen as an aggregation or combination of three components.

        S[t] = Seasonal signal
        T[t] = trend signal
        e[t] = noise signal

        these components can be combined either additively or multiplicatively to obtain the natural (observed)
        timeseries signal.
